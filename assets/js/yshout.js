...
YShout.prototype = {
	...
	initialLoad: function(updates) {
		...
		this.prefs.defaultNickname= "nickname";
		this.initForm();
		this.initRefresh();
		this.initLinks();
	...
	initForm: function() {
		...
		var postForm = 
			'<form id="ys-post-form"' + (this.prefs.inverse ? 'class="ys-inverse"' : '' ) + '><fieldset>' +
				'<input id="ys-input-nickname" placeholder="' + this.prefs.defaultNickname + '" type="text" accesskey="N" maxlength="' + this.prefs.nicknameLength + '" class="ys-before-focus" />' +
				'<input id="ys-input-message" placeholder="' + this.prefs.defaultMessage + '" type="text" accesskey="M" maxlength="' + this.prefs.messageLength + '" class="ys-before-focus" />' +
	...
	send: function() {
		...
		if (postMessage == '/cp')
			...
			this.ajax(this.updates, {
				reqType: 'post',
				angrezinickname: postNickname, // 2019-02-03 - ja editing key to make it less likely to have posts by bots
				angrezimessage: postMessage
			});
